...
updateGrid    = function(lobsterCoordinates){
  ...
  for( lobsterIndex in 1:numberOfLobsters ){
    ...
    if( distanceToTrap > 15){
      temp <- randomMove(xLobster = xOld , yLobster = yOld , dStep = 5)
      xNew[lobsterIndex] <- temp$EASTING
      yNew[lobsterIndex] <- temp$NORTHING
    }else{
      temp <- directionalMove(xLobster = xOld , yLobster = yOld , distanceToTrap = distanceToTrap, dStep = 5)
      xNew[lobsterIndex] <- temp$EASTING
      yNew[lobsterIndex] <- temp$NORTHING
    }
  }
...
random_point_sample_number=sum(initialGrid) # the number of points we want based on initial grid
